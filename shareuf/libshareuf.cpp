 ...
 * created with mode 0755/0644 on the resource filesystem so system
...
static irods::error shareuf_file_copy(
    int mode,
...
irods::error shareuf_generate_full_path(
...
} // shareuf_generate_full_path

// =-=-=-=-=-=-=-
/// @brief update the physical path in the file object
irods::error shareuf_check_path(
    ...
    if ( ( result = ASSERT_ERROR( data_obj.get(), SYS_INVALID_INPUT_PARAM, "Failed to cast fco to data_object." ) ).ok() ) {
        ...
        irods::error ret = shareuf_generate_full_path( _ctx.prop_map(),
...
irods::error shareuf_check_params_and_path(
    irods::plugin_context& _ctx ) {
    ...
    if ( ( result = ASSERT_PASS( ret, "resource context is invalid." ) ).ok() ) {
        result = shareuf_check_path( _ctx );
    }

    return result;

} // shareuf_check_params_and_path
...
irods::error shareuf_check_params_and_path(
    ...
    // verify that the resc context is valid
    ret = _ctx.valid();
    if ( ( result = ASSERT_PASS( ret, "shareuf_check_params_and_path - resource context is invalid" ) ).ok() ) {
        result = shareuf_check_path( _ctx );
    }
...
irods::error shareuf_file_mkdir_r(
    ...
    mode_t myMask = umask( ( mode_t ) 0000 );
            ...
            rodsLog( LOG_ERROR, "shareuf_file_mkdir_r ... " );
    ...
    while ( !done && result.ok() ) {
        ...
        if ( pos > 0 ) {
            ...
            rodsLog( LOG_NOTICE, "shareuf_file_mkdir_r '%s' made with mode %o",
...
irods::error shareuf_file_registered(
    irods::plugin_context& _ctx ) {
    irods::error result = SUCCESS();

    // Check the operation parameters and update the physical path
    irods::error ret = shareuf_check_params_and_path( _ctx );
...
irods::error shareuf_file_unregistered(
    irods::plugin_context& _ctx ) {
    irods::error result = SUCCESS();
...
irods::error shareuf_file_modified(
    irods::plugin_context& _ctx ) {
    irods::error result = SUCCESS();
...
irods::error shareuf_file_notify(
    irods::plugin_context& _ctx,
    const std::string* ) {
    irods::error result = SUCCESS();
    // Check the operation parameters and update the physical path
...
irods::error shareuf_file_getfs_freespace(
    irods::plugin_context& _ctx ) {
    irods::error result = SUCCESS();
...
irods::error shareuf_file_create(
    irods::plugin_context& _ctx ) {
    irods::error result = SUCCESS();
    ...
    if ( ( result = ASSERT_PASS( ret, "Invalid parameters or physical path." ) ).ok() ) {
        ...
        if ( kvp_str ) {
            ...
            if ( !ret.ok() ) {
                ...
                for ( ; itr != kvp.end(); ++ itr ) {
                    rodsLog(
                        LOG_DEBUG,
                        "shareuf_file_create_plugin - kv_pass :: key [%s] - value [%s]",
                        itr->first.c_str(),
                        itr->second.c_str() );
                } // for itr
            }
        ...
        ret = shareuf_file_getfs_freespace( _ctx );
        if ( ( result = ASSERT_PASS( ret, "Error determining freespace on system." ) ).ok() ) {
            ...
            if ( ( result = ASSERT_ERROR( file_size < 0 || ret.code() >= file_size, USER_FILE_TOO_LARGE, "File size: %ld is greater than space left on device: %ld",
                ...
                if ( fd == 0 ) {
                    ...
                    // make call to umask & open for create
                    mode_t myMask = umask( ( mode_t ) 0000 );
                    fd = open( fco->physical_path().c_str(), O_RDWR | O_CREAT | O_EXCL, 0644 );
                    errsav = errno;
                    if ( null_fd >= 0 ) {
                        close( null_fd );
                    }
                    rodsLog( LOG_NOTICE, "shareuf_file_create_plugin: 0 descriptor" );
...
} // shareuf_file_create_plugin

// =-=-=-=-=-=-=-
// interface for POSIX Open
irods::error shareuf_file_open(
    irods::plugin_context& _ctx ) {
    irods::error result = SUCCESS();
    ...
    if ( ( result = ASSERT_PASS( ret, "Invalid parameters or physical path." ) ).ok() ) {
        ...
        if ( kvp_str ) {
            ...
            if ( !ret.ok() ) {
                ...
                for ( ; itr != kvp.end(); ++ itr ) {
                    rodsLog(
                        LOG_DEBUG,
                        "shareuf_file_open_plugin - kv_pass :: key [%s] - value [%s]",
                        itr->first.c_str(),
                        itr->second.c_str() );
                } // for itr
            }
        ...
        if ( fd == 0 ) {
            ...
            rodsLog( LOG_NOTICE, "shareuf_file_open: 0 descriptor" );
        }
...
} // shareuf_file_read_plugin

// =-=-=-=-=-=-=-
// interface for POSIX Write
irods::error shareuf_file_write(
    irods::plugin_context& _ctx,
    void*                               _buf,
    int                                 _len ) {
    irods::error result = SUCCESS();
...
} // shareuf_file_write_plugin

// =-=-=-=-=-=-=-
// interface for POSIX Close
irods::error shareuf_file_close(
    irods::plugin_context& _ctx ) {
    irods::error result = SUCCESS();
...
} // shareuf_file_close_plugin

// =-=-=-=-=-=-=-
// interface for POSIX Unlink
irods::error shareuf_file_unlink(
    irods::plugin_context& _ctx ) {
    irods::error result = SUCCESS();
...
} // shareuf_file_unlink_plugin

// =-=-=-=-=-=-=-
// interface for POSIX Stat
irods::error shareuf_file_stat(
    irods::plugin_context& _ctx,
...
} // shareuf_file_stat_plugin

// =-=-=-=-=-=-=-
// interface for POSIX lseek
irods::error shareuf_file_lseek(
    irods::plugin_context& _ctx,
    long long                           _offset,
    int                                 _whence ) {
    irods::error result = SUCCESS();
...
} // shareuf_file_lseek_plugin

// =-=-=-=-=-=-=-
// interface for POSIX mkdir
irods::error shareuf_file_mkdir(
    irods::plugin_context& _ctx ) {
    irods::error result = SUCCESS();
...
} // shareuf_file_mkdir_plugin

// =-=-=-=-=-=-=-
// interface for POSIX rmdir
irods::error shareuf_file_rmdir(
    irods::plugin_context& _ctx ) {
    irods::error result = SUCCESS();
...
} // shareuf_file_rmdir_plugin

// =-=-=-=-=-=-=-
// interface for POSIX opendir
irods::error shareuf_file_opendir(
    irods::plugin_context& _ctx ) {
    irods::error result = SUCCESS();

    // =-=-=-=-=-=-=-
    // Check the operation parameters and update the physical path
    irods::error ret = shareuf_check_params_and_path< irods::collection_object >( _ctx );
    if ( !ret.ok() ) {
...
} // shareuf_file_opendir_plugin

// =-=-=-=-=-=-=-
// interface for POSIX closedir
irods::error shareuf_file_closedir(
    irods::plugin_context& _ctx ) {
    irods::error result = SUCCESS();
...
} // shareuf_file_closedir_plugin

// =-=-=-=-=-=-=-
// interface for POSIX readdir
irods::error shareuf_file_readdir(
    irods::plugin_context& _ctx,
...
} // shareuf_file_readdir_plugin

// =-=-=-=-=-=-=-
// interface for POSIX readdir
irods::error shareuf_file_rename(
    irods::plugin_context& _ctx,
    ...
    if ( ( result = ASSERT_PASS( ret, "Invalid parameters or physical path." ) ).ok() ) {
        ...
        ret = shareuf_generate_full_path( _ctx.prop_map(), _new_file_name, new_full_path );
        if ( ( result = ASSERT_PASS( ret, "Unable to generate full path for destination file: \"%s\".",
            ...
            ret = shareuf_file_mkdir_r( new_path.c_str(), mode );
...
} // shareuf_file_rename_plugin

// =-=-=-=-=-=-=-
// interface for POSIX truncate
irods::error shareuf_file_truncate(
    irods::plugin_context& _ctx ) {
    irods::error result = SUCCESS();

    // =-=-=-=-=-=-=-
    // Check the operation parameters and update the physical path
    irods::error ret = shareuf_check_params_and_path< irods::file_object >( _ctx );
    if ( ( result = ASSERT_PASS( ret, "Invalid parameters or physical path." ) ).ok() ) {
...
// is not used.
irods::error shareuf_file_stage_to_cache(
    irods::plugin_context& _ctx,
    ...
    if ( ( result = ASSERT_PASS( ret, "Invalid parameters or physical path." ) ).ok() ) {
        ...
        ret = shareuf_file_copy( fco->mode(), fco->physical_path().c_str(), _cache_file_name );
        result = ASSERT_PASS( ret, "Failed" );
    }
    return result;
} // shareuf_file_stagetocache

// =-=-=-=-=-=-=-
// unixSyncToArch - This routine is for testing the TEST_STAGE_FILE_TYPE.
// Just copy the file from cacheFilename to filename. optionalInfo info
// is not used.
irods::error shareuf_file_sync_to_arch(
    irods::plugin_context& _ctx,
    const char*                     _cache_file_name ) {
    irods::error result = SUCCESS();
    ...
    if ( ( result = ASSERT_PASS( ret, "Invalid parameters or physical path." ) ).ok() ) {
        ...
        ret = shareuf_file_copy( fco->mode(), _cache_file_name, fco->physical_path().c_str() );
        result = ASSERT_PASS( ret, "Failed" );
    }

    return result;

} // shareuf_file_sync_to_arch

// =-=-=-=-=-=-=-
// redirect_create - code to determine redirection for create operation
irods::error shareuf_resolve_hierarchy_create(
    irods::plugin_context& _ctx,
...
} // shareuf_resolve_hierarchy_create

// =-=-=-=-=-=-=-
// redirect_open - code to determine redirection for open operation
irods::error shareuf_resolve_hierarchy_open(
    irods::plugin_property_map&   _prop_map,
...
} // shareuf_resolve_hierarchy_open

// =-=-=-=-=-=-=-
// used to allow the resource to determine which host
// should provide the requested operation
irods::error shareuf_file_resolve_hierarchy(
    irods::plugin_context& _ctx,
        ...
        if ( ( result = ASSERT_ERROR( _opr && _curr_host && _out_parser && _out_vote, SYS_INVALID_INPUT_PARAM, "Invalid input parameter." ) ).ok() ) {
            ...
            if ( ( result = ASSERT_PASS( ret, "Failed in get property for name." ) ).ok() ) {
                ...
                if ( irods::OPEN_OPERATION  == ( *_opr ) ||
                    ...
                    ret = shareuf_resolve_hierarchy_open( _ctx.prop_map(), file_obj, resc_name, ( *_curr_host ), ( *_out_vote ) );
                    ...
                    ret = shareuf_resolve_hierarchy_create( _ctx, resc_name, ( *_curr_host ), ( *_out_vote ) );
...
} // shareuf_file_resolve_hierarchy

// =-=-=-=-=-=-=-
// shareuf_file_rebalance - code which would rebalance the subtree
irods::error shareuf_file_rebalance(
    irods::plugin_context& _ctx ) {
    return SUCCESS();

} // shareuf_file_rebalancec
...
irods::resource* plugin_factory( const std::string& _inst_name, const std::string& _context ) {
    ...
    resc->add_operation(
        ...
        function<error(plugin_context&)>(
            shareuf_file_closedir ) );

    resc->add_operation<const char*>(
        irods::RESOURCE_OP_STAGETOCACHE,
        function<error(plugin_context&, const char*)>(
            shareuf_file_stage_to_cache ) );
    ...
    resc->add_operation(
        ...
        function<error(plugin_context&)>(
            shareuf_file_registered ) );

    resc->add_operation(
        irods::RESOURCE_OP_UNREGISTERED,
        function<error(plugin_context&)>(
            shareuf_file_unregistered ) );

    resc->add_operation(
        irods::RESOURCE_OP_MODIFIED,
        function<error(plugin_context&)>(
            shareuf_file_modified ) );
